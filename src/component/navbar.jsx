...
const Nav = () => {
  ...
  return (
    <header className="bg-fuchsia-200 text-cyan-950 font-sans shadow-lg sticky top-0 left-0 w-full z-50">
      <nav className="flex justify-between items-center px-6 sm:px-8 lg:px-20 py-4 relative">
        ...
        <div className="hidden lg:flex flex-grow justify-center space-x-8">
          <ul className="flex gap-6 text-lg font-medium">
            ...
            {menuItems.map((menu, index) => (
              <li key={index} className="relative">
                <button
                  ...
                  aria-expanded={activeDropdown === menu.title ? "true" : "false"}
                >
                  {menu.title}
            ...
            <li><Link to="/news" className="hover:text-yellow-300">News & Events</Link></li>
          </ul>
        </div>
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden bg-purple-200 text-blue-950 shadow-lg px-8 py-6 flex flex-col items-center">
          <ul className="flex flex-col gap-4 text-lg font-medium w-full text-center">
            ...
            <li><Link to="/gallery" className="hover:text-yellow-300" onClick={closeMobileMenu}>Gallery</Link></li>
            <li><Link to="/news" className="hover:text-yellow-300" onClick={closeMobileMenu}>News & Events</Link></li>
          </ul>

          {/* Search Bar at Bottom in Mobile */}
          <input
            type="text"
            placeholder="Search..."
            value={searchTerm}
