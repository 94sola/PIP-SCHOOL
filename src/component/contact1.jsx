...
const Contact = () => {
  ...
  return (
    <div 
      ...
      style={{ backgroundImage: `url(${pip})` }} // Background image
    >
      {/* Blue Overlay */}
      ...
      <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-6 bg-gray-800 bg-opacity-60 px-8 md:px-12 lg:px-16 py-8 md:py-10 rounded-xl border border-white border-opacity-10 max-w-7xl w-full">
        <div className="w-full space-y-4 text-center lg:text-left">
          ...
          <div className="flex justify-center lg:justify-start space-x-4 mt-3">
            ...
            <a href="#" className="text-white text-2xl hover:text-gray-300"><FaPinterest /></a>
          ...
          <form className="space-y-3" onSubmit={handleSubmit}>
            <div className="flex flex-col sm:flex-row sm:space-x-3">
              <div className="w-full sm:w-1/2">
                <input type="text" name="firstName" placeholder="First Name*" value={formData.firstName} onChange={handleChange} className="w-full p-3 border-b border-gray-300 bg-transparent focus:outline-none" required />
              </div>
              <div className="w-full sm:w-1/2">
                <input type="text" name="lastName" placeholder="Last Name*" value={formData.lastName} onChange={handleChange} className="w-full p-3 border-b border-gray-300 bg-transparent focus:outline-none" required />
              </div>
            </div>
            <input type="email" name="email" placeholder="Email*" value={formData.email} onChange={handleChange} className="w-full p-3 border-b border-gray-200 bg-transparent focus:outline-none" required />
            ...
            <textarea name="message" placeholder="Leave us a message..." value={formData.message} onChange={handleChange} className="w-full p-3 border-b border-gray-200 bg-transparent focus:outline-none h-28" required></textarea>
